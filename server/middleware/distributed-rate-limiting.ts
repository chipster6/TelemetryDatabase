// Distributed Rate Limiting Middleware for API Security
...
// Helper class for distributed attempt tracking
class DistributedAttemptTracker {
  ...
  async getAttempts(identifier: string): Promise<{ count: number; firstAttempt: number; lockUntil?: number } | null> {
    if (redisService.isReady()) {
      try {
        const data = await redisService.get(this.getKey(identifier));
    ...
    const memoryMap = this.keyPrefix === 'login_attempts' ? loginAttempts : usernameAttempts;
    return memoryMap.get(identifier) || null;
  }

  async setAttempts(identifier: string, attempts: { count: number; firstAttempt: number; lockUntil?: number }): Promise<void> {
    if (redisService.isReady()) {
      try {
        const ttl = attempts.lockUntil ? Math.ceil((attempts.lockUntil - Date.now()) / 1000) + 300 : 3600; // Add 5 min buffer
        await redisService.set(this.getKey(identifier), JSON.stringify(attempts), ttl);
        return;
      } catch (error) {
        console.error('Redis set attempts error:', error);
        // Fall back to in-memory
      }
    }
    ...
    // Fallback to in-memory storage
    const memoryMap = this.keyPrefix === 'login_attempts' ? loginAttempts : usernameAttempts;
    memoryMap.set(identifier, attempts);
  }

  async deleteAttempts(identifier: string): Promise<void> {
    if (redisService.isReady()) {
      try {
        await redisService.del(this.getKey(identifier));
        return;
      } catch (error) {
        console.error('Redis delete attempts error:', error);
        // Fall back to in-memory
      }
    }
...
const ipTracker = new DistributedAttemptTracker('login_attempts');
const usernameTracker = new DistributedAttemptTracker('username_attempts');

// General API rate limiting (using express-rate-limit which has Redis support)
...
// Distributed account lockout mechanism
export const accountLockout = async (req: Request, res: Response, next: Function) => {
  ...
  try {
    const ipAttempts = await ipTracker.getAttempts(ip);
    const userAttempts = await usernameTracker.getAttempts(username);
    ...
    if (ipAttempts && (now - ipAttempts.firstAttempt) > attemptWindow) {
      await ipTracker.deleteAttempts(ip);
    }
    if (userAttempts && (now - userAttempts.firstAttempt) > attemptWindow) {
      await usernameTracker.deleteAttempts(username);
    }
...
// Track failed login attempts with distributed storage
export const trackFailedLogin = async (req: Request, res: Response, next: Function) => {
  ...
  try {
    // Track IP attempts
    const ipAttempts = (await ipTracker.getAttempts(ip)) || { count: 0, firstAttempt: now };
    ipAttempts.count += 1;
    ipAttempts.firstAttempt = ipAttempts.firstAttempt || now;

    // Track username attempts (prevents IP rotation bypass)
    const userAttempts = (await usernameTracker.getAttempts(username)) || { count: 0, firstAttempt: now };
    ...
    await ipTracker.setAttempts(ip, ipAttempts);
    await usernameTracker.setAttempts(username, userAttempts);
...
export const resetFailedAttempts = async (req: Request, res: Response, next: Function) => {
  const ip = req.userIP || req.ip || 'unknown';
  const username = req.username || req.body?.username || 'unknown';
...
// Initialize Redis service on module load
