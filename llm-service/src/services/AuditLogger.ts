...
export class AuditLogger extends EventEmitter {
  ...
  async getAuditTrail(userId: string, limit = 100): Promise<AuditEvent[]> {
    try {
      const eventIds = await this.redis.lrange(`audit:user:${userId}`, 0, limit - 1);
      const events = await Promise.all(
        eventIds.map(async (id) => {
          const data = await this.redis.get(`audit:event:${id}`);
  ...
  async getSecurityEvents(limit = 50): Promise<AuditEvent[]> {
    try {
      ...
      for (const type of securityTypes) {
        const eventIds = await this.redis.lrange(`audit:type:${type}`, 0, limit - 1);
        const events = await Promise.all(
          eventIds.map(async (id) => {
            const data = await this.redis.get(`audit:event:${id}`);
            return data ? JSON.parse(data) : null;
        ...
        allEvents.push(...events.filter(event => event !== null));
      }
  ...
  private prepareEventData(event: AuditEvent): any {
    ...
    if (this.config.encryptSensitiveData && this.encryptionKey && event.sensitive_data_accessed) {
      if (data.metadata) {
        data.metadata = { encrypted: this.encryptData(JSON.stringify(data.metadata)) };
      }
    }
  ...
  private shouldTriggerAlert(event: AuditEvent): boolean {
    return (this.config.enableRealTimeAlerts || false) && (
